...
- `arc rescue --transcript` reads a session through the `tapes` CLI when it is installed, falling back to arc's own claude/codex/pi readers otherwise, and names which reader answered (`arc-rescue/2`). This is what covers OpenCode sessions, and arc keeps working where `tapes` is not installed.
- `arc journal incident <topic>` records something that went wrong in the running of work rather than in the code, so a decision whose revisit trigger reads "when X happens" can fire on evidence. It is a record rather than work and never enters an actionable tier.
- `arc journal consume --decision <project>::<artifact>.md` cites a decision or conclusion in another registered project's journal, recording that project, the artifact's kind, and a digest of its bytes. A conclusion now resolves any non-discussion actionable artifact; a discussion still requires a decision.
- A change can declare that integration is not the goal: `arc begin --iterating` and `arc iterating <change> [--off]` record it in the ledger. `arc check` then reports a typed `iterating` blocker (exit 13) instead of a missing approval, `arc integrate` refuses and names the way out, and `arc inbox` buckets it separately. Schemas move to `arc-status/12` and `arc-inbox/5`.
- `arc workspace backlog --items` names every actionable artifact behind the counts — identity, kind, tier and per-item age — so a cross-project sweep is answerable in one invocation. JSON is `arc-workspace-backlog/2`.
- Brief readers now say when a brief's base revision no longer matches the change head: behind by N commits, or detached when the base is not in the change's history. Surfaced by `arc status`, `arc resume`, `arc rescue` and `arc brief`, and carried in the report as `brief.base_drift` (`arc-status/11`).
- `arc watch --until reviewed` returns when any verdict is recorded against the patchset under review, whatever it concluded, naming the verdict event. `ready` could not express this: a review asking for changes never satisfies it, so a wait on a dispatched review ran to timeout with "still working", "changes requested", and "reviewer died" indistinguishable.
- `arc verify --waive-dirty "<reason>"` lets evidence from a dirty worktree satisfy a gate, bound to the revision it was declared at so the next commit ends it, and reported by `arc status` only while it is in force. Recorded evidence now separates tracked from untracked dirt (`arc-status/10`), so the waiver reason is self-evident and the frequency of untracked-only wedges is measurable.
- `.arc/policy.toml` accepts `danger.source_roots` and `danger.acknowledged_safe`, closing danger classification over declared roots: `arc doctor` fails on a tracked file inside a root that is classified neither dangerous nor acknowledged-safe, and on one classified as both. Projects that declare no source root keep the open-world list they had.
- `arc keep --kind verified|rejected|constraint|hypothesis` records a fact the work discovered, and `arc resume` hands it back under `## Kept Context`, so a compacted or cold session does not re-derive it or re-try an approach already rejected. Carried on `arc status --json` as `kept`.
- `arc journal scaffolds [--show <name>] [--json]` lists every scaffold a write can prepend, marks which kinds default to one, and prints a scaffold's body before an append-only artifact commits to it; a repository template is shown as shadowing a built-in of the same name.
...
- Bare `arc` teaches identity before the first write (`SAY WHO YOU ARE`) and what to file when a session ends (`END A SESSION`); `arc env --help` names the session variables it detects and explains that its non-zero exit reports identity must be set by hand.
...
- A stacked change's patchset now records its declared opening base rather than the merge base against the integration target, so a stacked member's diff contains only what that member added instead of its whole ancestry.
- `arc doctor` checks danger classification against the whole repository, whatever directory it runs from. The scan previously ran `git ls-files` from the caller's directory, which lists only that subtree under names no declared source root matches, so the closed world silently stopped being checked and `doctor` reported nothing wrong. `src/commands/mod.rs` and `src/commands/history.rs` move to `danger.paths`: the first enforces append permissions and builds events, the second records the rewrite facts deciding whether a revision was moved or lost.
- A dirty-tree waiver excuses dirt that was observed, never cleanliness recorded as unknown. `verify --all --parallel` records cleanliness as unknown by design, and a waiver naming the same revision previously made that evidence green. `--waive-dirty` is now refused with `--parallel` rather than recording a waiver that could never apply.
- `arc journal position` and `arc journal answer` record the declared `--actor` on the typed event and name it in the heading when no model argued, so a person acting directly is no longer rendered `(unknown, ...)` and attributed to nobody. Where an actor is declared without a model, the heading now names the actor rather than the harness.
- `arc review` names a provisional approval in its verdict history, with the reason it was recorded and whether corroboration is still outstanding. The qualification was previously reported only when the verdict was written, so the read view showed a provisional approval as an unqualified one.
- `arc doctor` no longer hangs on repositories whose ledger records more revisions than a pipe buffer holds. The dangling-revision scan sent its whole query to `git cat-file` before reading any answer, deadlocking against it once the answers filled the pipe.
- The help-copy guard walks nested command groups, not only top-level commands, and reads clap's two-column layout; the twenty-eight undescribed flags and positionals it found under `forge`, `journal lane`, and `history resolve` are described. `arc finding --blocking` names which dispositions actually release the gate — resolved, accepted-risk, obsolete — rather than implying any disposition does.
- Every flag and positional in every command carries a description, guarded by a probe that walks the whole command list. A top-level kind name (`arc todo`, `arc handoff`) redirects to its journal verb instead of clap's spelling guess; `arc journal show <topic>` points at `arc journal latest`; and `arc fr` shares `arc journal feature-request`'s argument help by construction rather than restating it.
